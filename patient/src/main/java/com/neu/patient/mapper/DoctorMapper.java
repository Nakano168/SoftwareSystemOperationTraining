...
public interface DoctorMapper extends BaseMapper<Doctor> {
    @Select("SELECT d.*, u.real_name AS realName, COALESCE(q.available_quota, 0) AS availableQuota, " +
            "CASE WHEN COALESCE(q.available_quota, 0) > 0 THEN '可预约' ELSE '约满' END AS appointmentStatus " +
            "FROM doctor d " +
            ...
            "ORDER BY COALESCE(q.available_quota, 0) > 0 DESC, COALESCE(q.available_quota, 0) DESC, d.doctor_id")
